...
public abstract class ABMPreguntas extends BasePage {
	...
	private String[] tiposContenidoItemAsString = {
			ItemExamen.TiposItem.TEORICO.toString(),
			ItemExamen.TiposItem.PRACTICO.toString(),
			ItemExamen.TiposItem.TEORICOPRACTICO.toString() };

	public ABMPreguntas() {

		unidades = (String[]) MateriasPoolMock.getInstance().getUnidades(
				getMateria()).toArray(unidades);
	...
	private TiposItem getTipoContenidoItem() {
		...
		if (getTipoContenidoItemAsString().equals(
				ItemExamen.TiposItem.TEORICO.toString()))
			return ItemExamen.TiposItem.TEORICO;
		else if (getTipoContenidoItemAsString().equals(
				ItemExamen.TiposItem.PRACTICO.toString()))
			return ItemExamen.TiposItem.PRACTICO;
		else
			return ItemExamen.TiposItem.TEORICOPRACTICO;
	...
	public void onGuardar(IRequestCycle cycle) {
		...
		if (esPregunta())
			item = new ADesarrollar(getUnidad(), complejidad, getTextoItem(),
					getTipoContenidoItem());
		if (esEjercicio())
			item = new Ejercicio(getUnidad(), complejidad, getTextoItem(),
					getTipoContenidoItem());

		MateriasPoolMock.getInstance().setNewItem(getMateria(), item);

		//grabarPregunta en lugar de la linea de arriba
	...
	private void grabarPregunta(String pregunta, String materia,
			String unidadTematica, int complejidad,
			TiposItem tipo) {
		 MateriasPool.getInstance().getMateria(materia).addPregunta(new ADesarrollar(unidadTematica, complejidad,
				pregunta, tipo
				));
		 
				SQLServerMateriaDAO conexion = new SQLServerMateriaDAO();
				
			 try{	
				conexion.guardarPregunta( pregunta, materia,
						unidadTematica, complejidad,
						 tipo);
			}catch(Exception e){
